...
osMessageQueueId_t request_msgq_id;
osMessageQueueId_t result_msgq_id;
...
void Create_OS_Objects(void) {
	...
	request_msgq_id = osMessageQueueNew(4, sizeof(REQUEST_MSG_T), NULL);	
	result_msgq_id = osMessageQueueNew(4, sizeof(RESULT_MSG_T), NULL);
}
