...
uint32_t LCD_TS_Read(PT_T * position) {
	...
	REQUEST_MSG_T msg_x;
	RESULT_MSG_T 	dest_msg_x;
	
	REQUEST_MSG_T msg_y;
	RESULT_MSG_T 	dest_msg_y;
...
#if USE_ADC_FOR_TOUCHSCREEN
		msg_x.channelNum = LCD_TS_YU_CHANNEL;
		msg_x.resultQueue = request_msgq_id;
		status_x = osMessageQueuePut(request_msgq_id, 
					&msg_x, NULL, osWaitForever);	
#endif

		if(status_x == osOK){
			result_x = osMessageQueueGet(result_msgq_id, &dest_msg_x, NULL, osWaitForever);
			if (result_x==osOK) {
					yu = dest_msg_x.result;
			}
		}
		x = yu;
...
#if USE_ADC_FOR_TOUCHSCREEN
		msg_y.channelNum = LCD_TS_XL_CHANNEL;
		msg_y.resultQueue = request_msgq_id;
		status_y = osMessageQueuePut(request_msgq_id, &msg_y, NULL, osWaitForever);	
#endif

		if(status_y == osOK){		
			result_y = osMessageQueueGet(result_msgq_id, &dest_msg_y, NULL, osWaitForever);
			if (result_y==osOK) {
					xl = dest_msg_y.result;
			}		
		}
		y = xl;
